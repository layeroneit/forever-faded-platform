...
// GET /api/appointments - list (filtered by role: client=own, barber=own, owner=all at location(s))
appointmentsRouter.get('/', query('locationId').optional(), query('from').optional(), query('to').optional(), async (req, res) => {
  ...
  else if (req.role === 'barber' || req.role === 'manager') where.barberId = req.userId;
...
// POST /api/appointments - create (client or staff)
appointmentsRouter.post(
  '/',
  body('locationId').notEmpty(),
  body('clientId').optional(), // staff can set; client uses self
